function [ OutputImage ] = GrowRegionsMin( ImageData, ImageCentroids, FractCrit, MaxIntensity, UniqueCols, SameCols)
...
while (NumFinishedRegions < FractCrit*NumCentroids && totRegSize < totalImageElems)
    ...
    for i=find(~RegInf(:,5))'
        ...
        if (Intensity-Neighborhoods(i,I,4)*FIFO_WEIGHT) <= MaxIntensity
            % extract regsize
            RegSize = RegInf(i,4);
            % label it
